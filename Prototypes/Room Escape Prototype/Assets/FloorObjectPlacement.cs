...
public class FloorObjectPlacement : MonoBehaviour {
    ...
    void Update()
    {
        ...
        if (getTargetLocation(out point))
        {
            ...
            Vector3 prefabHalves = prefabPlacementObject.GetComponent<Renderer>().bounds.size / 2.0f;

            // Transform position is the center point of this object, x and z are grid slots from 0..slots-1
            int x = (int)Math.Round((point.x - transform.position.x + halfSlots.x - grid / 2.0f) / grid);
            int z = (int)Math.Round((point.z - transform.position.z + halfSlots.z - grid / 2.0f) / grid);
            ...
            Debug.Log(halfSlots.y);
            point.y = point.y + prefabHalves.y;

            // Create an object to show if this area is available for building
            // Re-instantiate only when the slot has changed or the object not inst antiated at all
