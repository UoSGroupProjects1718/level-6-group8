...
namespace UnityEditor.PostProcessing
{
    ...
    sealed class TrackballGroupDrawer : PropertyDrawer
    {
        ...
        Vector3 GetInput(Rect bounds, Vector3 hsv, float radius)
        {
            ...
            if (e.type == UnityEngine.EventType.MouseDown && GUIUtility.hotControl == 0 && bounds.Contains(mousePos))
            {
                if (e.button == 0)
                {
            ...
            else if (e.type == UnityEngine.EventType.MouseDrag && e.button == 0 && GUIUtility.hotControl == id)
            {
                e.Use();
                GUI.changed = true;
                GetWheelHueSaturation(relativePos.x, relativePos.y, radius, out hsv.x, out hsv.y);
            }
            else if (e.rawType == UnityEngine.EventType.MouseUp && e.button == 0 && GUIUtility.hotControl == id)
            {
                e.Use();
