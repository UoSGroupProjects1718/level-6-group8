    ...
    public sealed class CurveEditor
    {
        ...
        void OnCurveGUI(Rect rect, SerializedProperty curve, CurveState state)
        {
            ...
            for (int k = 0; k < length; k++)
            {
                ...
                {
                    // Keyframe move
                    if (m_EditMode == EditMode.Moving && e.type == UnityEngine.EventType.MouseDrag && isCurrentlySelectedCurve && isCurrentlySelectedKeyframe)
                    {
                        EditMoveKeyframe(animCurve, keys, k);
                    }

                    // Tangent editing
                    if (m_EditMode == EditMode.TangentEdit && e.type == UnityEngine.EventType.MouseDrag && isCurrentlySelectedCurve && isCurrentlySelectedKeyframe)
                    {
                    ...
                    if (e.rawType == UnityEngine.EventType.MouseUp && m_EditMode != EditMode.None)
                    {
        ...
        void OnGeneralUI(Rect rect)
        {
            ...
            if (e.type == UnityEngine.EventType.KeyDown && (e.keyCode == KeyCode.Delete || e.keyCode == KeyCode.Backspace))
            {
